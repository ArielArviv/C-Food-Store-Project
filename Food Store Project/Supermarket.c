...
int		initSuperMarket(SuperMarket* pMarket, const char* fileName,	const char* customersFileName, int isCompressed)
{
	...
	if (isCompressed == 1) {
		if (readMarketFromCompressedFile(pMarket, fileName, customersFileName) == 1)
		{
	...
	{
		if (readMarketFromFile(pMarket, fileName, customersFileName) == 1)
		{
...
{
	//this function also doing sorting//
	...
	while (pN != NULL)
	{
		...
		if (compRes > 0) {//found a place for new item, the next one bigger	
			if(!L_insert(pPrevNode, pProduct)) //giving the previous node for function
				return 0;
			return 1;
		}
...
Customer*	doPrintCart(SuperMarket* pMarket)
{
	...
	ShoppingCart* theCart = pCustomer->pCart;
	#ifdef DETAIL_PRINT 
		printShoppingCartWithBill(theCart);
	#else
		printShoppingCart(theCart);
	#endif	
	return pCustomer;
}
